...
import {iconTypeValidate,convertOldIconObject} from './helper/UtilityFunctions';
import canvasJson from './assets/abc';
...
function App() {
  ...
  useEffect(() => {
    ...
    let json=convertOldProject(canvasJson)
    canvas.loadFromJSON(json);
  }, [canvas]);

  const convertOldProject=(oldJson)=>{
   oldJson.objects.forEach((object,index)=>{
     if(iconTypeValidate(object)){
        let newIconObject={
          type:'icon',
          objects:[object],
      ...
      oldJson.objects[index]=newIconObject;
     }
   });
   return oldJson;
  }
  ...
  return <div className={'row mx-3'}>
    <div className={'col-1'}>
      ...
      <button onClick={()=>{activeObject.removeShape();canvas.renderAll()}}>Remove Shape</button>
    ...
    <div className={'col border rounded'} style={{height: '600px'}}>
      <canvas id={'canvass'} ref={canvasRef}/>
    </div>
    <div className={'col-3'}>
      {activeObject && activeObject.type==='icon'&&
      <>
